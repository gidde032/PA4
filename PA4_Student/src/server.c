...
void handle_search(int client_fd) {
  ...
  while (i < num_items) // searching inventory for items in query
  {
    if (strstr(inventory[i].name, query) &&
        count < MAX_RESULTS) { // checking if query is a substring of item
...
void handle_get_stock(int client_fd) {
  ...
  for (int i = 0; i < num_items;
       i++) { // loop through inventory checking for item
...
void handle_buy_item(int client_fd) {
  ...
  pthread_mutex_lock(&inventory_lock);
  for (int i = 0; i < num_items;
       i++) { // loop through inventory looking for item
    struct item *item = &inventory[i];
...
void *handle_client(void *arg) {
  ...
  while (1) {
    msg_enum msg;
    if (read(client_fd, &msg, sizeof(msg_enum)) <
        1) { // break if client disconnects
      break;
    }
